...
namespace MVC.WebApplication.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string? returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // Upon successfully create account, assign account to a default role.
        ...
        private async Task<bool> AssignRole(IdentityUser user, string role)
        {
            var DefaultRole = _roleManager.FindByNameAsync(role).Result;
            if (DefaultRole != null)
            {
                IdentityResult RoleAssignment = await _userManager.AddToRoleAsync(user, DefaultRole.Name);
            }
            else
            {
                ModelState.AddModelError(string.Empty, "Default role '" + role + "' does not exist in database! Abort assign this user to any role.");
                return false;
            }
            return true;
        }
